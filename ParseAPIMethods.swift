...
class ParseAPIMethods: NSObject {
    ...
    func updateStudentLocation(_ parameters: [String:AnyObject], completionHandlerForNewStudentLocation: @escaping (_ success: Bool, _ error: String?) -> Void) {
        
        // Step-2: Build the URL
        let request = NSMutableURLRequest(url: url(withPathExtension: Parse.Methods.StudentLocation + "/\(LocationViewController.objID)"))
        print("===============================\(request)")
